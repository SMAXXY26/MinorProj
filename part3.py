"""
part3.py  —  Temporal Smoother Training
========================================
...
Usage:
    python part3.py --sequences data/sequences/
    python part3.py --sequences data/sequences/ --resume
    python part3.py --demo      --source video.mp4
...
class TemporalSmoother(nn.Module):
    ...
    Input features per frame (7 values):
...
# =============================================================================
#  Feature Extraction — extract per-frame detection vectors from video
# =============================================================================
...
def extract_features_from_video(
    ...
    Returns list of dicts:
        {"features": np.array(T, 8), "labels": np.array(T,), "video": str}
    """
    nc = len(class_names)
...
def train(cfg, sequences, device, resume=False):
    tmp_cfg  = cfg["temporal"]
    ...
    for epoch in range(start_epoch, epochs):
        ...
        scheduler.step()
        val_loss, conf_acc, cls_acc = evaluate(model, val_loader, device, criterion)
...
# =============================================================================
#  Demo mode — test smoother on a single video without training
# =============================================================================
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Part 3 — Temporal Smoother")
    parser.add_argument("--config",     type=str, default="config/hyperparams.yaml")
    parser.add_argument("--sequences",  type=str, default="data/sequences",
                        help="Directory containing video files for training")
    parser.add_argument("--resume",     action="store_true")
    parser.add_argument("--demo",       action="store_true",
                        help="Demo mode — test smoother on a video")
    parser.add_argument("--source",     type=str, default="0",
                        help="Video source for demo mode")
    ...
    if args.demo:
        demo_smoother(args.source, cfg, device)
        sys.exit(0)

    print(f"\n{'═'*60}")
    print(f"  Part 3 — Temporal Smoother")
    print(f"{'═'*60}")

    # ── Load detector for feature extraction ─────────────────────────────
    det_ckpt = "logs/detector/best.pt"
    if not Path(det_ckpt).exists():
        print(f"[ERROR] Detector weights not found: {det_ckpt}")
        print("Run part1.py first.")
        sys.exit(1)
    ...
    print(f"\n  Extracting features from: {args.sequences}")
    sequences = extract_features_from_folder(
        args.sequences, detector,
        cfg["dataset"]["class_names"], cfg,
    )

    if len(sequences) == 0:
        print(f"""
  [INFO] No sequences found in {args.sequences}

  To train the temporal smoother you need video files.
  Add videos to:  {args.sequences}/

  Videos should contain weapon footage for positive examples
  and weapon-free footage for negative examples.
  ...
  Then run:  python part3.py --sequences data/sequences/
        """)
        sys.exit(0)

    print(f"  Total sequences : {len(sequences)}")
    total_frames = sum(len(s["features"]) for s in sequences)
    print(f"  Total frames    : {total_frames}")

    # ── Train ────────────────────────────────────────────────────────────
    train(cfg, sequences, device, resume=args.resume)
