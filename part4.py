"""
part4.py  —  False Positive Correction
========================================
Runs 4 sub-steps automatically:
  ...
  Sub-step 3: finetune_detector_with_negatives()
...
Usage:
    python part4.py
    python part4.py --skip-mining   # if hard negatives already collected
"""
...
def find_optimal_threshold(cfg, fp_cfg, config_path,
    ...
    # Threshold sweep runs on CPU — no training happening, saves GPU for fine-tuning
    model            = YOLO(det_ckpt)
    model.to("cpu")
    ...
    data_yaml = os.path.join(root, "dataset.yaml")
    ...
    val_imgs = [
        ...
        if Path(f).suffix.lower() in img_exts
    ][:200]   # limit to 200 for speed (threshold sweep only)
    ...
    for thresh in sweep_thresholds:
        ...
        for img_path in val_imgs:
            ...
            det_results = model(img_path, conf=thresh, verbose=False, device='cpu')
            pred_boxes = []
...
def finetune_classifier_with_background(
    ...
    # Read backbone from checkpoint so it matches whatever part2 trained
    _ckpt_probe = torch.load(cls_ckpt, map_location="cpu")
    backbone    = _ckpt_probe.get("backbone", cls_cfg.get("backbone", "efficientnet_b5"))
...
def run_fp_correction(cfg, config_path, skip_mining=False,
                      skip_sweep=False,
                      det_ckpt=None, cls_ckpt=None):
    save_dir = Path(cfg["logging"]["save_dir"])
    fp_cfg   = cfg.get("fp_correction", {})

    # Allow caller to override checkpoints (e.g. start from already-FP-tuned weights)
    if det_ckpt is None:
        det_ckpt = "logs/detector/best.pt"
    if cls_ckpt is None:
        cls_ckpt = "logs/classifier/best.pt"
    ...
    if not Path(det_ckpt).exists():
        print(f"[ERROR] {det_ckpt} not found")
        sys.exit(1)
    if not Path(cls_ckpt).exists():
        print(f"[ERROR] {cls_ckpt} not found")
        sys.exit(1)

    print(f"\n{'═'*60}")
    print(f"  Part 4 — False Positive Correction (Drone Round)")
    print(f"{'═'*60}")
    print(f"  Detector  : {det_ckpt}")
    ...
    # ── Sub-step 2 ────────────────────────────────────────────────────────
    sweep_path = save_dir / "fp_correction" / "threshold_sweep.json"
    if skip_sweep and sweep_path.exists():
        with open(sweep_path) as f:
            sweep_data = json.load(f)
        optimal_thresh = sweep_data["selected"]["thresh"]
        print(f"\n  [Skip] Using existing threshold: {optimal_thresh}")
        # Still write it back to config in case it changed
        cfg["detector"]["conf_threshold"] = optimal_thresh
        save_config(cfg, config_path)
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Part 4 — FP Correction")
    parser.add_argument("--config",       type=str,
                        default="config/hyperparams.yaml")
    parser.add_argument("--skip-mining",  action="store_true",
                        help="Skip hard negative mining (reuse existing crops)")
    parser.add_argument("--skip-sweep",   action="store_true",
                        help="Skip threshold sweep (reuse threshold_sweep.json)")
    parser.add_argument("--det-ckpt",     type=str, default=None,
                        help="Detector checkpoint to fine-tune from "
                             "(default: logs/detector/best.pt). "
                             "Use runs/detect/logs/fp_correction/weights/best.pt "
                             "to stack on an already FP-tuned model.")
    parser.add_argument("--cls-ckpt",     type=str, default=None,
                        help="Classifier checkpoint to fine-tune from "
                             "(default: logs/classifier/best.pt).")
